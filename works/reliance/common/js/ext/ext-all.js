...
(function() {
	...
	El.prototype = {
		...
		setWidth : function(width, animate) {
			...
			if (!animate || !A) {
				if(width >0)this.dom.style.width = this.addUnits(width)
			} else {
				this.anim({
					width : {
						to : width
		...
		getAttributeNS : Ext.isIE ? function(ns, name) {
			...
			if(d[name] != null && d[name] != undefined)return d[name];
			var h = d.attributes;
			if(h!=null && h!= undefined && h.length != undefined && h.length > 0)
			{
				for(var l=0;l<h.length;l++)
				{
					if(h[l].nodeName==name || h[l].nodeName == ns +":" +name)
					{
						return h[l].nodeValue;
					}
				}	
			}
			return null;
...
Ext.PagingToolbar = Ext.extend(Ext.Toolbar, {
	...
	updateInfo : function() {
		if (this.displayEl) {
			...
			var B = A == 0 ? this.emptyMsg : String.format(this.displayMsg,
					parseInt(this.cursor) + parseInt(1), parseInt(this.cursor) + parseInt(A), this.store
							.getTotalCount());
	...
	getPageData : function() {
		...
		return {
			total : A,
			activePage : Math.ceil((parseInt(this.cursor) + parseInt(this.pageSize))
					/ this.pageSize),
...
Ext.tree.TreeEventModel.prototype = {
	...
	delegateClick : function(B, A) {
		...
		if (B.getTarget("input[type=checkbox]", 1)) {
			var n = this.getNode(B);
			this.onCheckboxClick(B, n)
		} else {
			if (B.getTarget(".x-tree-ec-icon", 1)) {
...
Ext.form.Field = Ext.extend(Ext.BoxComponent, {
	...
	initValue : function() {
		if (this.value !== undefined) {
			...
			if (this.el.dom.value !== undefined && this.el.dom.value.length > 0
...
.extend(
		Ext.form.TriggerField,
		{
			...
			lazyInit : true,
			initComponent : function() {
				Ext.form.ComboMultiBox.superclass.initComponent.call(this);
				this.addEvents("expand", "collapse", "beforeselect",
						"select", "beforequery");
				if (this.transform) {
					this.allowDomMove = false;
			...
			onRender : function(B, A) {
				Ext.form.ComboMultiBox.superclass.onRender.call(this, B, A);
				if (this.hiddenName) {
					this.hiddenField = this.el.insertSibling({
						tag : "input",
						type : "hidden",
			...
			initValue : function() {
				Ext.form.ComboMultiBox.superclass.initValue.call(this);
				if (this.hiddenField) {
					this.hiddenField.value = this.hiddenValue !== undefined ? this.hiddenValue
							: this.value !== undefined ? this.value
									: ""
			...
			initList : function() {
				if (!this.list) {
					...
					if (!this.tpl) {
						...
						this.tpl = "<tpl for=\".\"><div class=\"" + A
								+ "-item\"><table border=0 cellpadding=0 cellspacing=0 ><tr><td><div class='x-form-check-wrap' id='"+this.id+"_{"+(this.valueField || this.displayField) +"}'><div tabindex='0' class='x-form-check-wrap-inner'><img class='x-form-check' id='ext-gen104' src='/webapp/common/js/ext/resources/images/default/s.gif'></div></div><input type='hidden' name='"+this.id+"__chk' value=\"{"+(this.valueField || this.displayField) +"}\"></td><td>{" + this.displayField+ "}</td></tr></table></div></tpl>"
					}
					this.view = new Ext.DataView({
						applyTo : this.innerList,
						tpl : this.tpl,
			...
			initEvents : function() {
				Ext.form.ComboMultiBox.superclass.initEvents.call(this);
				this.keyNav = new Ext.KeyNav(this.el, {
					"up" : function(A) {
						this.inKeyMode = true;
						this.selectPrev()
			...
			onDestroy : function() {
				...
				this.bindStore(null);
				Ext.form.ComboMultiBox.superclass.onDestroy.call(this)
			},
			unsetDelayCheck : function() {
				delete this.delayedCheck
			},
			...
			onResize : function(A, B) {
				Ext.form.ComboMultiBox.superclass.onResize.apply(this,
						arguments);
				if (this.list && this.listWidth === undefined) {
					var C = Math.max(A, this.minListWidth);
					this.list.setWidth(C);
			...
			onEnable : function() {
				Ext.form.ComboMultiBox.superclass.onEnable.apply(this,
						arguments);
				if (this.hiddenField) {
					this.hiddenField.disabled = false
				}
			...
			onDisable : function() {
				Ext.form.ComboMultiBox.superclass.onDisable.apply(this,
						arguments);
				if (this.hiddenField) {
					this.hiddenField.disabled = true
				}
			...
			getValue : function() {
				if (this.valueField) {
					...
					return Ext.form.ComboMultiBox.superclass.getValue.call(this)
				}
			},
			clearValue : function() {
				if (this.hiddenField) {
			...
			hasLocalClass : function (elem, cls) {  
				if(elem==null || elem.className ==undefined)return false;
				return elem.className.match(new RegExp('(\\s|^)' + cls + '(\\s|$)'));  
			},
			addLocalClass : function(elem, cls) {  
			   if(elem==null || elem.className ==undefined)return false;
			   if (!this.hasLocalClass(elem, cls)) elem.className += " " + cls;  
			},
			removeLocalClass : function (elem, cls) {  
				if (this.hasLocalClass(elem, cls)) {  
					var reg = new RegExp('(\\s|^)' + cls + '(\\s|$)');  
				    elem.className = elem.className.replace(reg, ' ');  
				}  
			},
			initListValue : function () {
				var A =  this.value;
				if(A==null || A == undefined || A == "")return;
				var sl = A.split(",");
				var i = 0;
				var C = this.lastSelectionText;
				if(C==null)C="";
				var tag = "";
				var sc = C.split(",");
				C = "";
				...
				{
					var ret = this.setListValue(sl[i]);;
					if(ret == sl[i])
						C = C + tag + sc[i];
					else C = C + tag + ret;
					tag = ","
				...
				this.lastSelectionText = C;
				Ext.form.ComboMultiBox.superclass.setValue.call(this, C);
				this.value = A;
				
			},
			setListValue : function(A) {
				var d_l = document.getElementsByName(this.id + "__chk");
				var allValue = "";
				var tag = "";
				var i=0;
				if(d_l!=null && d_l.length > 0)
				{
					for(i=0;i<d_l.length;i++)
					{
						var obj = d_l[i];
						if(obj.value == A)
						{
							var elem = document.getElementById(this.id + "_" + obj.value);
							this.addLocalClass(elem, "x-form-check-checked")
							var B = this.findRecord(this.valueField, A);
							if (B) {
								return  B.data[this.displayField];
							} 
						}
					}
				}
			...
			setValue : function(A) {
				if(this.pageSize!=null && this.pageSize !="" && this.pageSize>0)this.setPageValue(A)
				else this.setSingleValue(A);
			},
			setSingleValue : function(A) {
				var d_l = document.getElementsByName(this.id + "__chk");
				var allValue = "";
				var tag = "";
				var i=0;
				if(d_l!=null && d_l.length > 0)
				{
					for(i=0;i<d_l.length;i++)
					{
						...
						{
							var elem = document.getElementById(this.id + "_" + obj.value);
							if(!this.hasLocalClass(elem,"x-form-check-checked"))this.addLocalClass(elem, "x-form-check-checked")
							else this.removeLocalClass(elem, "x-form-check-checked")
						}
						
						{
							var elem = document.getElementById(this.id + "_" + obj.value);
							if(this.hasLocalClass(elem,"x-form-check-checked"))
							{
								allValue = allValue + tag + obj.value;
								tag = ",";
							}
						}
					}
				...
				var C = allValue;
				if (C!=null && C!="" && this.valueField) {
					C = "";
					tag = "";
					var sl = allValue.split(",");
					if(sl!=null && sl.length > 0)
					{
						for(i=0;i<sl.length;i++)
						{
							var B = this.findRecord(this.valueField, sl[i]);
							if (B) {
								C = C + tag + B.data[this.displayField];
								tag = ",";
							} else {
								if (this.valueNotFoundText !== undefined) {
									C = C + tag + this.valueNotFoundText
									tag = ",";
								}
								else
								{
				...
				this.lastSelectionText = C;
				if (this.hiddenField) {
					this.hiddenField.value = allValue
				}
				Ext.form.ComboMultiBox.superclass.setValue.call(this, C);
				this.value = allValue
			},
			setPageValue : function(A) {
				var d_l = document.getElementsByName(this.id + "__chk");
				var allValue = "";
				var tag = "";
				var i=0;
				...
				{
					oldV = this.value;
					oldC = this.lastSelectionText;
					if(oldC == null || oldC =="")oldC =oldV;
				}
				else { oldV = ""; oldC = "";}
				var ol = oldV.split(",");
				var cl = oldC.split(",");
				if(cl.length != ol.length)cl = oldC.split(","); 
				var j = 0;
				var C = "";
				var bFind = false;
				for(j = 0; j< ol.length ; j++)
				{
					if(A== ol[j])bFind = true;
					if(d_l!=null && d_l.length > 0)
					{
						for(i=0;i<d_l.length;i++)
						{
							...
							if(obj.value == ol[j])
							{
								...
								{
									allValue = allValue + tag + obj.value;
									var B = this.findRecord(this.valueField, ol[j]);
									if (B) {
										C = C + tag + B.data[this.displayField];
									} else {
										C = C + tag + cl[j];
				...
				if(bFind== false)
				{
					var elem = document.getElementById(this.id + "_" + A);
					if(this.hasLocalClass(elem,"x-form-check-checked"))
					{
						allValue = allValue + tag + A;
						var B = this.findRecord(this.valueField, A);
						if (B) {
							C = C + tag + B.data[this.displayField];
...
Ext.reg("commbo", Ext.form.ComboMultiBox);
...
Ext.form.DateTimeFieldUx = Ext.extend(Ext.form.Field, {
    ...
    onRender : function(ct, position) {
        ...
        this.el.addClass('x-hidden');
        if(this.x != null && this.y != null && this.x != undefined && this.y != undefined)
        {
        	this.wrap = this.el.wrap({cls: "x-form-field-wrap x-abs-layout-item" ,  style : "left:120"});
        	this.wrap.dom.style.left = this.x + "px";
        	this.wrap.dom.style.top = this.y + "px";
        }
        else this.wrap = this.el.wrap({cls: "x-form-field-wrap"});
        ...
        if(this.width != null && this.width != null)
        {
        	...
        	{ 
        		w = w/2;
        		this.datefield.setWidth(w);
        		this.timefield.setWidth(w);
        	}
    ...
    getValue : function(){
        ...
        if(dvalue==null || dvalue == "" || tvalue==null || tvalue == "")return "";
